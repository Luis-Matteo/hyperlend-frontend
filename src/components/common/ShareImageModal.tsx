...
function ShareImageModal({
  ...
  const downloadImage = () => {
    if (selectedIndex === null || imageRefs.current[selectedIndex] === null) return;

    toPng(imageRefs.current[selectedIndex]!, { cacheBust: true })
      .then((dataUrl) => {
          ...
          <CardItem className="px-5 sm:px-10 py-8 max-h-[calc(100vh-50px)] overflow-auto">
            <div className="flex flex-col justify-center items-center gap-5 text-center">
              <div className="flex flex-col gap-3 justify-center items-center">
                ...
                <p className="font-nexa font-black text-xl text-secondary">Share your position</p>
              </div>
              <div className="flex flex-col justify-center items-center gap-2">
                <p className="font-lufga text-sm font-light text-secondary">X (Twitter) username{' '}</p>
                <input
                  autoFocus
                  type="text"
                  ...
                  style={{ background: 'transparent', outline: 'none', boxShadow: 'none', width: 'auto', minWidth: '50px' }}
                />
              </div>
              <div className="flex justify-center items-center gap-4 text-white text-sm">
                <p>Use X (Twitter) profile photo?</p>
                <ToggleButton status={fetchTwitterPhoto} setStatus={setFetchTwitterPhoto} />
                <motion.img
                  src={refreshIcon}
                  alt="refresh"
