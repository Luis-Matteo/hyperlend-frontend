        ...
        >
            <CardItem className='py-3 px-2 md:px-6 flex justify-between items-center gap-2'>
                ...
                >
                    {hypervaultStatus.map((item) => (
                        <motion.button
                            ...
                            whileTap={{ scale: 0.95 }}
                            type='button'
                            className={`py-2 px-7 font-lufga rounded-full text-xs font-bold transition-all duration-500 ${item.disabled ? 'text-gray' : status === item.slug ? 'bg-secondary' : ' text-white'
                                }`}
                ...
                >
                    ...
                    >
                        <motion.p
                            className='text-secondary font-lufga capitalize'
                            layout
                        >
                    ...
                    {mobileStatus && (
                        <div className='absolute top-16 left-0 w-full flex flex-col bg-primary rounded-lg shadow-2xl'>
                            {hypervaultStatus.map((item) => (
                                <button
                                    key={item.slug}
                                    className={`py-4 px-7 font-lufga rounded-lg text-xs font-bold transition-all duration-500 ${item.disabled ? 'text-gray' : ' text-secondary hover:bg-secondary/20 '
                                        }`}
                                    disabled={item.disabled}
                                    onClick={() => {
                                        setStatus(item.slug);
