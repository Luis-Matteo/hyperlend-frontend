      ...
      {selectedPair ? (
        ...
        >
          <div className='relative flex justify-between items-center'>
            <button
              className='left-0 flex gap-2 items-center'
            ...
            <div className='flex gap-4 items-center'>
              <div className='flex -space-x-3'>
                <img
                  ...
                  src={
                    pairs.find((item) => item.id == selectedPair)?.token1.icon
                  }
                  alt='token'
                />
                <img
                  className='w-10 h-10 object-cover rounded-full'
                  src={
                    pairs.find((item) => item.id == selectedPair)?.token2.icon
                  }
                  alt='token'
                />
              </div>
              <p className='text-secondary text-lg font-lufga'>
                {pairs.find((item) => item.id == selectedPair)?.token1.name} /{' '}
                {pairs.find((item) => item.id == selectedPair)?.token2.name}
              </p>
            </div>
          ...
          <div className='flex flex-col gap-3 mt-8'>
            <div className='bg-[#071311] rounded-2xl'>
              <p className='px-5 pt-5 text-sm italic text-[#E1E1E1]'>
                Health Factor
              </p>
            ...
            <div className='w-full py-6 bg-[#071311] rounded-2xl flex flex-col justify-center items-center gap-2'>
              <p className='text-secondary font-lufga'>
                ${formatNumber(98915548, 2)}
              </p>
              <p className='text-secondary/40 text-sm font-lufga'>
                Position Value
              </p>
            </div>
        ...
        >
          <div className='mt-4 flex justify-between items-center'>
            <p className='text-secondary text-opacity-40 text-sm font-lufga'>
              Pairs
            </p>
            <p className='text-secondary text-opacity-40 text-sm font-lufga'>
              Position Value
          ...
          <div className='flex flex-col max-h-[400px] overflow-auto'>
            {pairs.map((item) => (
              <button
                className='flex justify-between items-center py-3 border-b-[1px] border-[#212325] hover:bg-secondary/10'
                key={item.id}
