      ...
      >
        <div className=''>
          <div className='absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 rounded-full flex justify-center items-center'>
            {dots.map((_, index) => {
              const angle = (index / dots.length) * (180 * (length / (length - 1))) - 90;
          ...
          >
            <span className={`font-bold text-3xl ${healthFactor > 1.3 ? 'text-white' : 'text-error'}`}>
              {isNaN(healthFactor) ? 0 : formatNumber(healthFactor, 2)}
            </span>
          </div>
          <div className='absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-[#2B2B2B] shadow-[0px_0px_30px_#252B3B]'
            style={{
              width: size - 70,
              height: size - 70
            }}
